...
export default function index({ navigation, route }) {
  ...
  async function success() {
    ...
    let Call = parseFloat(state.currentStepCount * conversationFactor).toFixed(
      0
    );
        ...
        >
          {num == 0 ? (
            ...
            <View style={styles.viewimg}>
              <Text style={styles.texthead}>เวลาที่ใช้ไป</Text>
              <Text style={styles.texttime}>
                {formatTime(h)}:{formatTime(m)}:{formatTime(s)}
              </Text>
              <Text style={styles.texthead}>อุปกรณ์ที่เชื่อมต่อ</Text>
              <View style={styles.touch}>
                {route?.params?.devices == "HeathKit" && (
                  <Image
                ...
                {route?.params?.devices == "SOSORUNPODV2" && (
                  <Image
                    source={{
              ...
              <TouchableOpacity
                onPress={async () => {
                  ...
                  if (route?.params?.devices == "SOSORUNPODV2") {
                    ...
                    setVisible1(false);
                    if (
                      count - route?.params?.last_totals >=
                      dataEV.distance[0]
                    ) {
                      success();
                    } else {
              ...
              >
                {num == 5 ? (
                  <Text style={styles.go}>ส่งผลวิ่ง</Text>
                ) : (
                  <TouchableOpacity
